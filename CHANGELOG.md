...
* **mcq-digitizer:** structured papers now also have a real Test mode -- same "Answer the quiz" flow as MCQ, but the A/B/C/D options are replaced by one combined text box per question for the student's full working. Each question is submitted (and graded) individually rather than all at once at the end -- silently, no score shown until the results screen, same as MCQ never reveals anything mid-quiz either -- and can still be edited and resubmitted afterward; the final "Submit quiz" only has to grade whatever wasn't already submitted (or was edited since). Submitting sends each question's own mark-scheme crop image (never its extracted text -- see the bug fix below) plus the student's answer to a vision-capable free OpenRouter model for grading. A question only counts as correct toward the score if it earned FULL marks; the exact score it did earn (e.g. "1 / 2") is still shown on every result row, it just doesn't count as a pass short of full marks -- matching how an MCQ question is either the right letter or it isn't, no partial credit there either.
