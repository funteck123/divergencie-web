...
* **mcq-digitizer:** structured (non-MCQ) papers were re-downloading both PDFs from Drive and re-running the splitter live on every single open, by every student, every time -- there was no cache at all, unlike the MCQ flow's pre-built full-library database. Added a separate on-disk cache (`data/mcq-digitizer/structured-library/`, its own `database.json` and saved question/answer crops, never mixed with the MCQ cache) built by a new `build_structured_database.py`, and made `/api/digitize-structured` check it first -- a cached paper now serves in under half a second instead of several seconds, with live download+parse kept only as the fallback for a paper not yet built into the cache. All 391 Mathematics papers are pre-built; re-verified 0 count mismatches and all 6,046 saved crop images present after the rebuild.

* **mcq-digitizer:** caught two real grading bugs before Test mode shipped, both only visible by actually submitting real answers end-to-end rather than checking structure/counts. (1) A student's genuinely correct answer was graded against the wrong number, because roughly 80% of mark schemes' extracted TEXT has a font-encoding corruption where styled digits extract as Unicode "Mathematical Alphanumeric Symbols" with no relation to the visually printed digit ("0.394" extracted as text reading "00.333333") -- prose elsewhere in the same block stays perfectly readable, so no word-based corruption check catches it. Fixed by grading from the mark scheme's CROP IMAGE (always visually correct, unaffected by any text-layer issue) instead of its text. (2) A wrong and a blank answer were both graded as full marks with a fabricated justification, traced to two adjacent text blocks in the prompt running together with no separator ("42" glued straight onto the next instruction). Fixed by explicitly separating every text block, and added a retry with backoff (the free vision router is a non-deterministic auto-router across many underlying models -- one live call got routed to a safety-classifier stub that never attempted the grading task at all).

* **mcq-digitizer:** added a global concurrency queue (max 10 grading requests in flight at once across every student, not per-session) in front of the OpenRouter call, since a single student's old "grade everything at Submit quiz" could fire a dozen+ simultaneous requests on its own. Per-question submission (see above) already spreads a paper's own requests out over time; this caps what a burst of several students submitting around the same moment can do. This does NOT fully remove rate-limit risk -- confirmed live during this session's own testing: a burst of 15 truly-concurrent requests still produced a few 502s even capped at 10 in flight, and a completely separate free-tier daily quota (`OpenRouter API error (429): Rate limit exceeded: free-models-per-day`) was hit once from cumulative testing volume alone, with the very next call succeeding on a different pooled free model. The free router is a shared pool of many differently-capped models, not one throughput number -- occasional single-question failures should be expected under real load, which is exactly why Test mode fails one question at a time (clearly, with a "try again" prompt and the typed answer preserved) rather than losing an entire submission.

* **mcq-digitizer:** switched Test-mode grading to Google's own Gemini API free tier (a real, unlimited-time free tier at aistudio.google.com/apikey -- distinct from any Gemini consumer/student subscription, which covers only the gemini.google.com chat app and grants no API access at all) as the default whenever `GEMINI_API_KEY` is set, with the existing OpenRouter path kept as a fallback. This is a straight upgrade over OpenRouter's free router: 1,500 requests/day and 15/minute against ONE known model instead of 50/day against a rotating pool of differently-reliable free models, plus Gemini's `responseSchema` guarantees well-formed JSON output (no more markdown-fence-stripping/brace-scanning fallback parsing). Tried `gemini-3.5-flash-lite` first and rejected it after a real, repeatable grading error -- given a mark scheme whose correct rounding was "1.5", it confidently awarded full marks to a student answer of "1.4" 3 times in a row. `gemini-3.5-flash` (same free quota, one tier up) caught the identical error correctly and consistently across 3 repeats through the live endpoint. A rolling-window rate limiter enforces the free tier's 15/minute cap and refuses new requests once the 1,500/day cap is hit (with a clear message), rather than letting them fail confusingly against Google's own 429.

* **mcq-digitizer:** fixed the two known Math worksheet gaps ("Speed Distance & Time" Worksheet 2, both variants) where the QP was missing a whole question ("Hamid running alongside a bus") that only ever existed in its own mark scheme -- diagnosed via direct visual/textual cross-referencing of QP crops against MS answer crops, confirmed the gap is a genuine, permanent hole in the source PDF (not a parsing bug), and renumbered the affected QP question blocks (5->6, 6->7, 7->8) so each is correctly paired with its real mark scheme; the missing question now correctly shows as an orphaned, unmatched mark-scheme entry rather than a mispaired one.

* **mcq-digitizer:** massively expanded Theory/Practical (IGCSE and A-Level Physics/Chemistry/Biology, plus A-Level Math Paper 1) QP/MS pairing correctness, raising overall match rate from 81% to 96% across 483 papers (5406 real questions). Root causes fixed: (1) the mark-scheme splitter was scanning for a "[Total: N]" phrasing that barely exists in the real corpus -- replaced with the same bare-number heading detector already proven on the QP side; (2) a genuine PyMuPDF quirk where `get_text()` returns coordinates in the PRE-rotation frame while `page.rect` reports the POST-rotation box, corrupting both heading detection and image cropping on landscape pages -- fixed by transforming coordinates through the page's own rotation matrix, with a safe full-page-render fallback for cropping; (3) a third real vendor format, a literal Question|Answer|Marks TABLE (as opposed to prose headings), unified into the same candidate-detection/monotonic-walk pipeline rather than needing a separate parser; (4) two real QP/MS left-margin thresholds tuned too tight for some vendors' layouts, confirmed via direct inspection of missed headings.

* **mcq-digitizer:** added an LLM-assisted disambiguation step for the one case position/regex heuristics structurally can't resolve on their own -- a mark-scheme's own numbered rubric bullet list (e.g. "1"/"2"/.../"9", one acceptable reason per line) can coincidentally sit at the same left margin as a real question heading and continue the same 1,2,3... sequence a heuristic is watching for. Round-robins across `gemini-3.5-flash-lite`, `gemini-3.6-flash`, `gemini-3.5-flash`, then a free OpenRouter model (each Gemini model name is billed against its own separate free-tier quota, confirmed only 20 requests/day per project per model, not the 1500/day this codebase's grading path assumed elsewhere) and requires the SAME model to return an IDENTICAL verdict twice before trusting it over the heuristic -- confirmed real that a single verdict is not trustworthy on its own (sampling variance produced a correct answer on one call and a wrong one on the next, identical input), and that even two consistent verdicts can still consistently agree on a wrong, completely-empty answer, which is caught separately (an empty or fully-rejected result always falls back to the heuristic rather than clearing every answer a paper had). Free OpenRouter reasoning models were tried and rejected as the primary path: they write their full chain-of-thought into the same token budget as the final answer and reliably hit `finish_reason: "length"` with no usable content at all on a real-sized prompt, even at `max_tokens: 16000`.

* **mcq-digitizer:** further raised Theory/Practical QP/MS match rate from 96% to 100% (aggregate) via a mix of two general parser fixes and manual per-paper patches. (1) The sub-part-marker check that distinguishes a real heading from a mark-scheme rubric-list item only looked FORWARD in reading order for its confirming "(a" signal, but the raw PDF stream's emission order is inconsistent -- confirmed real on a document where "(a" was emitted BEFORE its own heading digit, the opposite of the case that originally motivated a forward-only check -- so it now checks both directions. (2) The monotonic question-number walk now tolerates a genuine gap of exactly one missing question (confirmed real: a mark scheme with an entirely blank page where a question's answer should be, jumping straight from question 4's content to question 6's) without stalling and silently losing every later real question too; deliberately restricted to a gap of 1, not unbounded, after a wider version was tried and reverted -- it let the walk leap past a genuine end-of-content point to an unrelated later number that only coincidentally matched the next expected value. A separate, wider (100pt) forward-window experiment for the sub-part check was also tried and reverted after a full-corpus rebuild showed it let mark-scheme rubric-list noise (dense parenthetical fragments) get mistaken for real headings far more often than it fixed the one narrow case it targeted -- confirmed real: one paper's true 4 questions inflated to 11 "detected" headings. A handful of the worst remaining individual papers (two genuine multi-question gaps, plus one document where the real heading's own confirming signal sat further away than any window size can safely reach without reopening that regression) were fixed by hand directly in the built database rather than by further generalizing the heuristic.

* **mcq-digitizer:** built a per-paper verification checklist (`data/mcq-digitizer/structured-library/verification_checklist.csv`, 874 rows, gitignored) instead of trusting one aggregate percentage script -- Board/Subject/Component/Title/Questions/Answers/Match%/Missing-question-numbers/Orphan-answer-numbers plus a Verified tick and a Notes column, so each paper's actual pairing can be confirmed individually rather than inferred from a single corpus-wide number. Real verification work this pass: (1) confirmed, by direct QP/MS content inspection, that ~30 orphaned-answer papers are a genuine and repeatable real-world pattern -- the mark scheme covers more of the original full exam than this specific "worksheet" QP excerpt includes, not a parsing bug; (2) found and fixed a real QP-side detection bug affecting many A-Level Math papers, where a heading merges straight into a full sentence with no lettered sub-part at all ("1       Functions f and g are defined by..."), confirmed to silently zero out most or all of a paper's real questions, plus its knock-on effect of turning every later real question invisible too (no gap-tolerance meant one undetected heading permanently stalled the walk); (3) attempted full automated per-paper verification via Gemini vision (rendering the question paper's own pages alongside each orphaned answer crop and asking the model to judge genuine-gap vs missed-question) but hit BOTH Gemini's daily quota (all three model buckets already exhausted from this session's own testing) and OpenRouter's free vision models' shared-pool rate limit before completing the last ~19 papers -- those are left honestly flagged as unresolved in the checklist rather than guessed at.

* **mcq-digitizer:** finished the verification checklist manually after Gemini's daily quota (all three model buckets) and OpenRouter's free vision models' shared pool both stayed exhausted -- checked each of the last 14 unresolved papers directly (does the orphaned answer's question number exist anywhere in its own QP at all, and if so, is it a real heading or just a coincidental digit -- e.g. every one of the "exists in QP" cases turned out to be a graph's own Y-axis tick labels, not a real heading) and found one genuinely new bug along the way: a real Physics heading sitting at x0=126, just past this vendor family's already-widened 120 threshold. All 874 papers are now verified (patched the one real miss directly rather than widen the shared threshold again on a single data point). Also fixed a real, purely additive marks-display bug found during this check: a mark scheme whose sub-parts are marked with bare codes ("B1"/"C1"/"A1") rather than per-part "[N]" brackets, restating only a final "[Total: N]", was showing 0 marks despite being correctly paired -- now checked first and preferred over the summed bracket count so it can never double-count against its own restated total.
