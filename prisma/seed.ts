...
async function main() {
  ...
  const sess1 = await prisma.academicSession.create({ data: {
  ...
  const sess2 = await prisma.academicSession.create({ data: {
  ...
  await prisma.academicSession.create({ data: {
  ...
  // HR candidates
  await prisma.candidate.create({ data: { name: "Sarah Miller", email: "sarah.miller@example.com", role: "A Level Teacher",     status: "Interview",  cvLink: "https://drive.google.com/cv-sarah", notes: "Strong Imperial background", outreach: "LinkedIn" }});
  await prisma.candidate.create({ data: { name: "Linda Chen",   email: "linda.chen@example.com",   role: "IGCSE Maths Teacher", status: "Offer Sent", cvLink: "https://drive.google.com/cv-linda", notes: "Expected join June 1",       outreach: "IG"       }});
