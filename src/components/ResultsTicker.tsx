...
export default function ResultsTicker() {
  return (
    <div className="w-full bg-white border-b border-[var(--border-subtle)] flex items-stretch h-14 overflow-hidden relative">
      {/* Fixed Part (Left) */}
        ...
        <div className="flex gap-16 animate-infinite-scroll whitespace-nowrap px-8 items-center">
          {[...results, ...results, ...results].map((item, idx) => (
            <div key={idx} className="flex items-center gap-4 group">
              ...
              <span className="text-xs font-black text-[var(--text-muted)] tracking-widest uppercase group-hover:text-[var(--navy)] transition-colors">
                {item.uni}
              </span>
