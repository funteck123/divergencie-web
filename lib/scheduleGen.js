import { nextId, insertScheduleItemsDedup } from "./db";
  ...
  for (const service of db.services) {
    ...
    for (const batch of batches) {
      for (const occ of batch.OccuranceList || []) {
        ...
        // A concurrent request may have already generated one of these
        // exact Occurrence+Date slots between this function's read of
        // `existing` above and now -- insertScheduleItemsDedup() lets the
        // database itself (not in-memory JS) decide which candidates
        // actually land, closing the race this loop's own "is it missing"
        // check can't see across two separate serverless invocations.
        if (candidates.length > 0) {
          await insertScheduleItemsDedup(db, candidates);
