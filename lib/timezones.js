...
// TKT-0277: converts a ScheduleItem's real calendar Date ("YYYY-MM-DD") +
// "HH:MM" from the item's own stored Timezone (the class's reference zone)
// to the viewer's own Timezone. Same offset math as convertWeeklyTime
// above, but anchored to the ACTUAL stored date instead of "the next
// occurrence of this weekday from today" -- a dated ScheduleItem already
// has its real date, so there's no weekday-to-anchor step needed here.
// Returns the possibly-shifted Date too (a late-night class can roll to
// the next or previous calendar date in a timezone further east/west).
export function convertScheduleDateTime(date, time, fromTz, toTz) {
  const [y, mo, d] = (date || "").split("-").map(Number);
  const [h, m] = (time || "").split(":").map(Number);
  if (!Number.isFinite(y) || !Number.isFinite(h) || !Number.isFinite(m)) return { date, time };
  if (!fromTz || !toTz || fromTz === toTz) return { date, time };

  // Sample fromTz's offset at midday on the stored date -- safely clear of
  // any DST transition -- then apply it to the actual wall-clock time.
  const middayUTC = new Date(Date.UTC(y, mo - 1, d, 12, 0, 0));
  const fromOffset = offsetMinutesAt(middayUTC, fromTz);
  const utcInstant = new Date(Date.UTC(y, mo - 1, d, h, m, 0) - fromOffset * 60000);

  const toOffset = offsetMinutesAt(utcInstant, toTz);
  const toLocal = new Date(utcInstant.getTime() + toOffset * 60000);

  return {
    date: `${toLocal.getUTCFullYear()}-${String(toLocal.getUTCMonth() + 1).padStart(2, "0")}-${String(toLocal.getUTCDate()).padStart(2, "0")}`,
    time: `${String(toLocal.getUTCHours()).padStart(2, "0")}:${String(toLocal.getUTCMinutes()).padStart(2, "0")}`,
  };
}

// Real 3-letter abbreviations for this school's fixed-offset common zones --
// Intl's own {timeZoneName: "short"} refuses to give these (e.g. Asia/Kolkata
// comes back "GMT+5:30", not "IST"), since it avoids ambiguous codes. These
// five don't observe DST, so a plain lookup is safe; Europe/London does
// observe it, so it's resolved from the actual UTC offset on the given date
// instead of hardcoded.
const TZ_ABBR_COMMON = {
  "Asia/Kolkata": "IST",
  "Asia/Riyadh": "AST",
  "Asia/Karachi": "PKT",
  "Asia/Dubai": "GST",
...
// Short timezone label to show next to a displayed time, e.g. "IST" -- for
// any zone outside this school's common list, falls back to whatever Intl
// itself reports (a real abbreviation for many zones, e.g. "EDT", or a
// "GMT+N" offset when the zone has no standard short code).
export function tzAbbrFor(date, timeZone) {
  if (!timeZone) return "";
  const [y, m, d] = (date || "").split("-").map(Number);
  if (!Number.isFinite(y) || !Number.isFinite(m) || !Number.isFinite(d)) return "";
  const instant = new Date(Date.UTC(y, m - 1, d, 12, 0, 0));
  if (timeZone === "Europe/London") return offsetMinutesAt(instant, timeZone) > 0 ? "BST" : "GMT";
  if (TZ_ABBR_COMMON[timeZone]) return TZ_ABBR_COMMON[timeZone];
  return (
