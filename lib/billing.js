...
import { getRateToINR, convertINRAmount } from "./fxRates";

export const BILLING_TYPES = ["Monthly", "Hourly", "OneOff"];

// Shared by app/api/invoices/route.js and app/api/trial-enroll/route.js
// (TKT-0105): both need the same OneOff-vs-LineItems invoice shape
// branching, so it lives here rather than being duplicated or imported
// route-file-to-route-file.
export function studentCurrencyOf(db, studentId) {
...
export async function lineItemINR(db, lineItem, year, month) {
...
export async function refreshStudentTotal(db, invoice) {
...
// TKT-0035: `assumePresentIfUnlogged` — when a scheduled session has NO
// attendance record at all (neither the subject nor a co-enrolled
// Teacher/Student logged it — see TKT-0037), should that session still
// count as attended for billing purposes? Deliberately asymmetric by
// design, not a bug: a Student is billed for a class that happened even if
// nobody got around to logging it (true by default for invoices — see
// app/api/invoices/route.js); Staff are only paid for hours someone
// actually logged (false for paychecks — app/api/paychecks/route.js —
// unchanged from prior behavior). Only affects a session with ZERO
// records; a session with at least one real record (from anyone) always
// uses that record's own LoggedDuration exactly as before, regardless of
// this flag.
// TKT-0104: `fullFeeMode` -- Management wants attendance-based proration
// switched off "for now" (invoices only, all billed the full advance fee
// regardless of attendance; paychecks unaffected). A toggle rather than a
// deletion, per their explicit ask, so it can go back to attendance-based
// billing later by just passing false/omitting it again -- the actual
// proration math below is untouched, this only picks which branch runs.
// scheduledHours/attendedHours/hasUnresolvedAttendance are still computed
// and returned exactly as before either way: attendance tracking and its
// conflict-review flow both keep working regardless of billing mode.
export function computeHoursAndAmount(db, { userId, serviceId, batchId, year, month, assumePresentIfUnlogged = false, fullFeeMode = false }) {
  ...
  // Per-session, not a flat sum over accepted records — a session with
  // zero records needs its own explicit assumePresentIfUnlogged decision,
  // not just an absence from the sum (which would silently agree with
  // "false" regardless of the flag).
  let attendedHours = 0;
  for (const item of monthScheduleItems) {
    const recordsForSession = attendanceForUser.filter((a) => a.ScheduleItemID === item.ScheduleID);
    if (recordsForSession.length === 0) {
      if (assumePresentIfUnlogged) attendedHours += Number(item.Duration) || 0;
      continue;
    }
    const accepted = recordsForSession.find((a) => a.AcceptedForBilling !== false) || recordsForSession[0];
    attendedHours += Number(accepted.LoggedDuration) || 0;
  }
  ...
  let amount;
  if (fullFeeMode) {
    // TKT-0104: bill the full advance fee regardless of attendance --
    // Hourly's "full" is the scheduled hours (what was booked), not what
    // was actually attended. OneOff was already attendance-independent.
    if (billingType === "Hourly") {
      amount = rate * scheduledHours;
    } else {
      amount = rate;
    }
