...
const KEY = "dcp1_user";
const IMPERSONATOR_KEY = "dcp1_impersonator";
...
// Mirrors the server's impersonatorUserId session claim (see
// lib/session.js's impersonationCookieFor) purely so DashboardShell can
// show a "you're logged in as X — Stop impersonating" banner client-side.
// This is a UI convenience only, never trusted for auth: the httpOnly
// cookie is what the server actually checks on every request.
...
export function setImpersonatorInfo(info) {
  if (info) localStorage.setItem(IMPERSONATOR_KEY, JSON.stringify(info));
...
// TKT-0027: today's date as "YYYY-MM-DD" (local calendar day, not UTC —
// matches how ScheduleItem.Date is already stored/compared everywhere
// else, e.g. lib/scheduleGen.js's own fmtDate) — the shared "hide past
// schedule/attendance entries by default" cutoff used across every
// dashboard's schedule list (Teacher/Student/Staff/Ambassador/Parent/
// Management).
...
// TKT-0279: "yyyy-mm-dd" for N days before today, for the schedule table's
// "Last N days" range presets.
...
export async function api(path, options = {}) {
  // TKT-0199: a FormData body (file upload) must NOT get a manual
  // Content-Type -- fetch sets one itself with the correct multipart
  // boundary. Forcing application/json here would silently break the
  // server's req.formData() parse.
  ...
  //
  // TKT-0023: /api/login itself is the one exception — its 401 means "wrong
  // username/password", not "your session died" (there was never a session
  // to expire). Without this carve-out, a wrong-password attempt hit this
  // branch, wiped localStorage, force-navigated to /login, and returned a
  // Promise that never resolves — so the login form's own catch(err) never
  // ran and "Invalid username or password" was never shown; the page just
  // silently reloaded itself with the form cleared.
