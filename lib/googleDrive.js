...
// TKT-0199: uploads a real file into a Drive folder via OAuth (the
// read-only API key above can't write). Multipart upload -- Drive's
// simplest write path for an arbitrary binary blob with metadata attached
// in one request. Returns the new file's webViewLink.
export async function uploadFile({ accessToken, folderId, filename, mimeType, buffer }) {
  const boundary = "dc_upload_boundary_" + Math.random().toString(36).slice(2);
  ...
  if (!res.ok) throw new Error(`uploadFile: Drive upload failed: ${JSON.stringify(data)}`);
  return data;
}
