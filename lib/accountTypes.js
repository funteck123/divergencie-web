...
// Services only actually bill in a handful of currencies in practice — the
// Rate currency picker uses this trimmed list instead of every ISO 4217
// code, so it is not cluttered with currencies nobody sets up a Service
// rate in. This does not restrict billing itself: any currency in the full
// CURRENCIES list can still be entered via the API, and the FX rate lookup
// (lib/fxRates.js) covers every one of them regardless of this list.
export const RATE_CURRENCIES = CURRENCIES_FULL.filter((c) =>
  ["INR", "SAR", "GBP", "USD", "AED", "BHD", "PKR", "BDT", "AUD"].includes(c.code)
);
