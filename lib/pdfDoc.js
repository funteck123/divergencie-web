...
//   balanceLabel, currency, balance (Balance Due box — the outstanding
//   amount, already converted into the payer's own currency by the caller),
//   lineItems: [{ item, quantity, rate, amount, currency }] (each line's own
//   native billed currency/amounts — not converted; this is the invoice's
//   own record of what was actually charged. `currency` is per-line since a
//   multi-line invoice could in principle bill different services in
//   different currencies; omit it and the row just shows a bare number),
//   discountPercent (no taxPercent — TKT-0093 removed the always-0% Tax line),
//   total, totalCurrency (the final Total line — the full charge, already
//   converted into the payer's own currency; a separate currency from
//   `currency` above since either conversion can independently fall back to
//   the native billed currency if a rate lookup fails),
//   terms
// }
export async function drawDocumentPDF(data) {
  ...
  for (let i = 0; i < TABLE_ROWS; i++) {
    ...
    if (item) {
      ...
      const itemCurrencyPrefix = item.currency ? `${item.currency} ` : "";
      ctx.fillText(`${itemCurrencyPrefix}${Number(item.rate).toFixed(2)}`, colX.amount - 8, y + 16);
      ctx.fillText(`${itemCurrencyPrefix}${Number(item.amount).toFixed(2)}`, tableRight - 6, y + 16);
    }
    y += rowH;
  }

  // TKT-0093: Tax was always hardcoded to 0% at every call site — this
  // business doesn't charge tax, so the line was pure clutter/confusion,
  // never a real figure. Removed; Discount stays (not part of this ticket).
  let sy = y + 30;
  ctx.font = "10px Roboto";
  const summaryRows = [["Discount:", `${data.discountPercent}%`]];
  ...
  ctx.fillStyle = "#222222";
  rightText(ctx, `${data.totalCurrency || data.currency} ${Number(data.total).toFixed(2)}`, MARGIN_R, sy);
...
//
// data: {
//   company, registeredAs?, regNo?, period, emplNo, name, department, role,
//   currency (the actual billed currency — drives Gross/Nett Pay/Employer SOCSO),
//   staffCurrency? (the staff's own home/payroll currency, shown only in the
//   header "Currency:" row — falls back to `currency` if omitted),
//   icPassport, epfNo, socsoNo, taxNo,
//   earnings: [{ item, quantity, rate, amount }], grossPay,
//   deductions: [{ item, quantity, rate, amount }], nettPay,
//   employerSocso,
//   convertedTotal?: { currency, amount },
//   showYtd?: boolean (default false — Year-to-Date section is off unless explicitly enabled),
//   ytd: [{ label, value }],
// }
...
export async function drawPayslipPDF(data) {
  ...
  const headerRows = [
    ...
    ...(data.registeredAs ? [["Registered As:", data.registeredAs]] : []),
    ...(data.regNo ? [["Reg. No.:", data.regNo]] : []),
    ["Period:", data.period],
    ["Empl No.:", data.emplNo],
    ["Name:", data.name],
    ["Department:", data.department],
    ["Role:", data.role],
    ["Currency:", data.staffCurrency || data.currency],
  ...
  function payrollTable(title, rows, totalLabel, totalValue) {
    ...
    rows.forEach((r, i) => {
      ...
      const rowCurrencyPrefix = r.currency ? `${r.currency} ` : "";
      ctx.fillText(`${rowCurrencyPrefix}${Number(r.rate).toFixed(2)}`, colRate, y + 12);
      ctx.fillText(`${rowCurrencyPrefix}${Number(r.amount).toFixed(2)}`, colAmount, y + 12);
      y += lineH;
    });
    if (rows.length === 0) {
      psRow(ctx, MARGIN_L, y, PS_TABLE_W, lineH, null);
  ...
  // Optional second total when the payslip's own currency (what the rate
  // was actually billed in) differs from the staff's own Currency — e.g. a
  // Staff enrollment billed in USD, but the person is paid/tracked in INR.
  // Not a second deduction/earning, just a converted-total reference line.
  if (data.convertedTotal) {
    psRow(ctx, MARGIN_L, y, PS_TABLE_W, lineH, PS_GRAY_LIGHT);
    ctx.fillStyle = PS_TEXT;
    ctx.font = "bold 10px Roboto";
    ctx.textAlign = "left";
    ctx.fillText(`Total (${data.convertedTotal.currency})`, MARGIN_L + 8, y + 12);
    ctx.textAlign = "right";
    ctx.fillText(`${data.convertedTotal.currency} ${Number(data.convertedTotal.amount).toFixed(2)}`, colAmount, y + 12);
    y += lineH;
  }

  // YEAR-TO-DATE PAYROLL DETAIL — off by default (data.showYtd truthy to
  // turn back on); kept intact rather than deleted since this was only
  // switched off temporarily, not dropped as a feature.
  if (data.showYtd) {
    y += 24;
    psRow(ctx, MARGIN_L, y, PS_TABLE_W, sectionH, PS_GRAY);
    ctx.fillStyle = PS_TEXT;
  ...
  // TKT-0092: the payslip had no contact email anywhere, unlike the
  // invoice PDF's own Terms footer.
  y += 20;
  ctx.fillStyle = "#666666";
  ctx.font = "9px Roboto";
  ctx.textAlign = "left";
  ctx.fillText("For any queries about this payslip, contact finance@divergencie.co.uk.", MARGIN_L, y);
