...
//   convertedTotal?: { currency, amount } — an extra "Total (<currency>)"
//   line shown below Total, for when the payer's own currency differs from
//   the currency this document was actually billed/paid in.
// }
export async function drawDocumentPDF(data) {
  ...
  if (data.convertedTotal) {
    ...
    ctx.fillStyle = BROWN_DARK;
    ctx.font = "bold 10px Roboto";
    rightText(ctx, `Total (${data.convertedTotal.currency}):`, MARGIN_R - 90, sy);
    ctx.fillStyle = "#222222";
    rightText(ctx, Number(data.convertedTotal.amount).toFixed(2), MARGIN_R, sy);
  }
