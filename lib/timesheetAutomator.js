...
// input: { name, batch?, currency?, rate?, link?, course? } -- name is the
// only required field, matching the template's own required column (Batch/
// Currency/Rate/Link/Course are all real optional fields on the template).
// Returns { fileId, fileName, url }.
export async function createTimesheet(input) {
  const { name, batch = "", currency = "", rate = "", link = "", course = "" } = input;
  ...
  return { fileId, fileName, url: `https://docs.google.com/spreadsheets/d/${fileId}/edit` };
}
