...
export async function writeDB(db) {
  // Each table's own upsert-then-delete must stay in order (a table's rows
  // shouldn't be deleted before the current set is written), but the 12
  // tables are otherwise independent — running them one after another was
  // taking ~6.5s per write (a real timeout risk); Promise.all cuts that to
  // roughly the slowest single table's round trip.
  await Promise.all(
