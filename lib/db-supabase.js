...
export async function insertScheduleItemsDedup(rows) {
  ...
  if (error) {
    // PGRST202: the RPC doesn't exist yet -- data/tmp/migration_dedup_
    // schedule_generation.sql hasn't been run in this environment. Rather
    // than hard-crash every /api/me, /api/schedule, and /api/services call
    // (this function's only callers) until someone remembers to run that
    // migration, fall back to the plain upsert every OTHER collection
    // already uses -- reintroduces the original race window this function
    // exists to close, but a rare duplicate schedule row is a far smaller
    // failure than every dashboard 500ing outright. Once the migration
    // runs, this branch stops firing on its own, no code change needed.
    if (error.code === "PGRST202") {
      const { error: fallbackError } = await supabase.rpc("upsert_rows", { p_table: table, p_rows: payload });
      if (fallbackError) throw new Error(`[db-supabase] insertScheduleItemsDedup fallback: ${table}: ${fallbackError.message}`);
      invalidateReadCache();
      return new Set(payload.map((row) => row.id));
    }
    throw new Error(`[db-supabase] insertScheduleItemsDedup: ${table}: ${error.message}`);
  }
  invalidateReadCache();
