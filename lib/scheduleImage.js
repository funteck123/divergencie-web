...
// equivalent). Keyed by entity.role (student/teacherRole/staff), not the
// raw UserType/Group string.
const ROLE_CELL_COLOR = {
  student: "#3b82f6",
  teacherRole: "#22c55e",
  staff: "#f59e0b",
};
...
export async function drawSchedule(entity, entries) {
  ...
  for (let row = 0; row < numRows; row++) {
    ...
    for (let col = 0; col < NUM_COLS; col++) {
      ctx.fillStyle = ROLE_CELL_COLOR[entity.role] || "#f9f5ff";
      ctx.fillRect(colLeft(col), rowTop, GRID_COL_WIDTH, rowHeight - dividerHeight);
    }
  }

  ctx.fillStyle = ROLE_CELL_COLOR[entity.role] ? "white" : "black";
...
// tiny row.
const ADMIN_PALETTE = ["#7b2cbf", "#2c7bbf", "#2cbf7b", "#bf7b2c", "#bf2c5e", "#5e2cbf", "#2cbfaa", "#bfab2c"];
...
// entries: [{ day, time, duration?, serviceName, facilitator? }] — every
  ...
  for (let r = 0; r < rowTimes.length; r++) {
    ...
    for (let c = 0; c < NUM_COLS; c++) {
      ...
      cellEntries.forEach((entry, idx) => {
        ...
        ctx.fillStyle = ADMIN_PALETTE[idx % ADMIN_PALETTE.length];
        ctx.fillRect(x + 2, blockY + 1, dayColW - 6, blockH - 2);
